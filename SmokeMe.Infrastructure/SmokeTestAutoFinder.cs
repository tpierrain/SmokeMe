...
using TypeExtensions = SmokeMe.Helpers.TypeExtensions;
...
namespace SmokeMe
{
    ...
    public class SmokeTestAutoFinder : IFindSmokeTests
    {
        ...
        public SmokeTestInstanceWithMetaData[] FindAllSmokeTestsToRun(params string[] requestedCategories)
        {
            ...
            foreach (var smokeTestType in types)
            {
                ...
                var smokeTestCategories = TypeExtensions.GetCategories(smokeTestType);
                if (smokeTest != null)
                {
            ...
            return smokeTestInstances.ToArray();
        }
